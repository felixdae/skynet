...
function _M.recv_frame(sock_id, max_payload_len, force_masking)
    --local data, err = sock:receive(2)
    local data, err = socket.read(sock_id, 2)
    if not data then
...
--    end
    ...
    elseif payload_len == 127 then
        ...
        local data, err = socket.read(sock_id, 8)
        if not data then
    ...
    if rest > 0 then
        ...
        data, err = socket.read(sock_id, rest)
        if not data then
...
function _M.send_frame(sock_id, fin, opcode, payload, max_payload_len, masking)
    ...
    local bytes, err = socket.write(sock_id, frame)
    if not bytes then
